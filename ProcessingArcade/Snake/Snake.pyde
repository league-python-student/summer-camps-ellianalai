...
def setup():
    ...
    snake_head = Segment(400, 300)
    
    # 3. Initialize the 'food' variable using the Food class to make new food
    food = Food()
    # 4. Use the food's drop() method to place a piece of food randomly in the window.
    food.drop()
...
def draw():
    ...
    if collision(snake_head,food):
        ...
        new_snake_segment = Segment(snake_head.x, snake_head.y)
